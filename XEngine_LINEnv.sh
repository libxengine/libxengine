...
function InstallEnv_CheckIns()
{
	...
	if [ "$m_EnvRelease" -eq "2" ] ; then
		...
		if  [ "$VERSION_ID" == "20.04" ]; then
			m_EnvAPT+=" libmongoc-1.0-0 libbson-1.0-0 libsrt-dev libfontconfig1-dev"
		elif [ "$VERSION_ID" == "22.04" ]; then
    		m_EnvAPT+=" libmongoc-1.0-0 libbson-1.0-0 libzimg-dev libplacebo-dev libdav1d-dev libaom-dev libfontconfig-dev libgme-dev"
		elif [ "$VERSION_ID" == "24.04" ]; then
    		m_EnvAPT+=" libmongoc-1.0-0t64 libbson-1.0-0t64 libsnappy-dev libopenmpt-dev libcdio-dev libjxl-dev libiec61883-dev libavcodec-dev libavdevice-dev libavfilter-dev libavformat-dev libswresample-dev libswscale-dev libffmpeg-nvenc-dev"
		else
