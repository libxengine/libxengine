...
返回值
  类型：逻辑型
  意思：是否成功
备注：内部有计时器,如果发送请求后中间有断点,可能会造成时间异常
*********************************************************************/
extern "C" bool NTPProtocol_Parse_Header(LPCXSTR lpszMSGBuffer, int nMSGLen, __int64x * pInt_TimeOffset, __int64x * pInt_TimeDelay, NTPPROTOCOL_TIMEINFO * pSt_RefTime = NULL, NTPPROTOCOL_TIMEINFO * pSt_RecvTime = NULL, NTPPROTOCOL_TIMEINFO * pSt_OriginTime = NULL, NTPPROTOCOL_TIMEINFO * pSt_TrsTimestamp = NULL, int* pInt_TimePoll = NULL, int* pInt_TimePrecision = NULL, XCHAR * ptszIDAddr = NULL);